...
welcome() {
  ...
  /___/_//_/_/\__/           /___/\__/_/  |___/\__/_/
     ...
     Ubuntu & Rocky Linux 8,9 & CentOS 7 初始化脚本
        ...
        Version: ${scriptVer}    Update: 2023-11-13
        By: 大威(Davy)    System: ${Purple}${OS} ${OS_VER}
        ${Color_off}"
}
...
countdown() {
  ...
  while [ $num -gt 0 ]; do
    echo -ne "\r        ${Cyan}初始化脚本 ${BRed}$num${Cyan} 秒后开始, 按 ${Green}ctrl C ${Cyan}取消${Color_off}"
    sleep $delay
    ((num--))
  done

  echo -e "\r        ${Cyan}初始化脚本 ${BRed}0${Cyan} 秒后开始, 按 ${Green}ctrl C ${Cyan}取消${Color_off}"
}

cmdCheck() {
  if ! hash "$1" >/dev/null 2>&1; then
    error "Command [${BRed}${1}${Color_off}] not found"
    return 1
  fi
}
...
yumInstall() {
  info "安装 ${1}"
  cont "进行中..."
  if sudo yum install -y $1 >/dev/null; then
    success "${1} 安装完成。\n"
  else
    warn "${1} 安装失败！\n"
  fi
...
update_source_for_china() {
  ...
  1)
    if [[ "$OS" == *"Ubuntu"* ]]; then
      ...
      if [[ "$OS" == **"Rocky"** ]]; then
        ...
        for file in $config_files; do
          sudo cp "$file" "/etc/yum.repos.d/backup/$(basename "$file" | sed 's/.repo/.bak$(date +%Y%m%d-%H%M%S)/')"
    ...
    if [[ "$OS" == **"Rocky"** ]]; then
      ...
      # 备份并修改配置文件
      cont "[tsinghua清华] 备份并修改 ${Purple}$OS${Color_off} 的 epel 配置文件"
...
timezone_setting() {
  ...
  if ! timedatectl | grep "Asia/Shanghai"; then
    cont "当前系统时区为：${Green}$curr_TZ${Color_off}..."
    cont "设置系统时区为：${Green}Asia/Shanghai${Color_off}..."
    sudo timedatectl set-timezone Asia/Shanghai

  else
    warn "当前系统时区为：${Green}Asia/Shanghai${Color_off},跳过时区设置。"
  fi
  ...
  if [[ $OS == **"CentOS"** || $OS == **"Rocky"** ]]; then
    ...
    if grep -q "^pool" /etc/chrony.conf; then
      ...
      sudo sed -i '/^pool/s/.*/pool cn.ntp.org.cn iburst\npool ntp.aliyun.com iburst\npool ntp.tencent.com iburst\n&/' /etc/chrony.conf
    else
      # 在文件末尾添加新的 NTP 服务器
      sudo sed -i '/^server/s/.*/server cn.ntp.org.cn iburst\nserver ntp.aliyun.com iburst\nserver ntp.tencent.com iburst\n&/' /etc/chrony.conf
    fi
  ...
  success "系统时区已设为：${Green}Asia/Shanghai${Color_off} 并${Green}开启${Color_off}时间同步。\n"
}
...
install_golang() {
  ...
  if [ -x "$(command -v wget)" ]; then
    wget https://golang.google.cn/dl/go1.15.15.linux-amd64.tar.gz -P /usr//src/
  else
...
help() {
  echo "Usage: bash InitSrvr.sh [type] [target] [options]"
  echo
  ...
  echo
  echo "	Update System"
  echo "		bash InitSrvr.sh update"
  echo "	Install java maven"
  echo "		bash InitSrvr.sh java meven"
  echo "	Install all python tools"
  echo "		bash InitSrvr.sh python"
}
...
main() {
  if [ $# -eq 0 ]; then
    ...
    echo "Usage: bash InitSrvr.sh [type] [target] [options]"
    echo
    echo "InitSrvr.sh [-h|--help] [-v|--version] [-b|--basic]"
